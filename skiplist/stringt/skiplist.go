...
func (th *Skiplist) updateSearchFingers(key string, fingers []*node) (err error) {
	startLevel := th.level - 1
	startNode := th.headNode

	if fingers[0] != nil && len(fingers[0].key) > 0 {
		if less, err := th.compare(fingers[0].key, key); err != nil {
			...
			for l := 1; l < th.level; l++ {
				if fingers[l].next[l] != nil && len(fingers[l].key) > 0 {
...
func (th *Skiplist) Insert(key string, value uint32) (*node, error) {
	if len(key) == 0 {
...
func (th *Skiplist) Select(key string) (iter *Iterator, err error) {
	return th.SelectRange(key, key)
}

func (th *Skiplist) SelectRange(key1, key2 string) (iter *Iterator, err error) {
...
func (th *Skiplist) SelectFrom(key1 string) (iter *Iterator, err error) {
	if len(key1) == 0 {
		return nil, errors.New("skiplist/SelectRange: key1 or key2 is nil")
	}
...
func (th *Skiplist) SelectFromBitmap(key1 string) (bm *idx.Bitmap, found bool, err error) {
	if len(key1) == 0 {
		return nil, false, errors.New("skiplist/SelectRange: key1 or key2 is nil")
	}

	if th.compare == nil {
		return nil, false, errors.New("skiplist/SelectRange: comparator is not set (== nil)")
	}
...
func (th *Skiplist) DeleteValue(key string, v uint32) bool {
