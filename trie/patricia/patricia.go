...
// DeleteUint32 deletes the item represented by the given prefix.
//
...
func (trie *Trie) Delete(key Prefix) (deleted bool) {
	...
	for ; i >= 0; i-- {
		if current := path[i]; current.item != 0 || current.children.length() >= 2 {
			break
		}
	}
...
func (trie *Trie) empty() bool {
	return trie.item == 0 && trie.children.length() == 0
}
